...
if __name__ == '__main__':
   ...
   #TODO: CLEANUP video_number_list = [1, 2, 3, 4, 5]
    video_number_list = [1]
    ...
    for video_number in args.video_number_list:
        ...
        # Save scores to .txt
        data = json.load(open(os.path.join(path_output, f'{video_name}_scores.json'),'rb'))
        data_list = []
        for key, value in data.items():
           data_list.append([key, value[0] if len(value) > 0 else 0])

        with open(os.path.join(path_output, f'{video_name}_scores.txt'),'w') as file:
